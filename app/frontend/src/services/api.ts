...
export const api = {
  ...
  getAgents: async (): Promise<Agent[]> => {
    try {
      const response = await apiFetch('/hedge-fund/agents');
      if (!response.ok) {
  ...
  getLanguageModels: async (): Promise<LanguageModel[]> => {
    try {
      const response = await apiFetch('/language-models/');
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const data = await response.json();
  ...
  saveJsonFile: async (filename: string, data: any): Promise<void> => {
    try {
      const response = await apiFetch('/storage/save-json', {
        method: 'POST',
        headers: {
